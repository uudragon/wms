...
#####7.	套餐定义接口
该接口用于套餐的创建与更新。如果所传入的package_code已经在数据库中有对应记录，则按该package_code更新已有纪录；
否则，按新纪录进行创建。
注意：如果是update，则当前package_code关联的ProductPackageDetails表中的产品记录会被删除，并按照新提交的数据重新插入到ProductPackageDetails表中。
######2.1 url

	method：POST
	wms/baseinfo/package/save/
	注意：结尾的’/’不能省略
...
名称 | 类型 | 是否必填 | 说明
------------ | ------------- | ------------ | ----------
package_code|String	|Y	|套餐码，唯一
package_name|String	|Y	|套餐名称
package_desc|String|N|套餐描述
details	|Array|	Y|套餐产品明细，
creator|String|Y|创建人
updater|String|Y|修改人
yn|Int|Y|是否失效。默认1。1有效；0失效
...
样例报文：
	...
	{‘error’:’Attribute[‘package_code’] can not be none.’}


#####8.	单笔套餐查询接口
该接口按照指定的套餐号查询对应的套餐及其明细信息。
######8.1 url
	method: GET
	wms/baseinfo/package/${package_code}/
	注意：结尾的’/’不能省略, ${package_code}为套餐号
######8.2 header
	Content_Type:application/json;charset=utf-8
	Accept:application/json
######8.3 请求参数
...
名称 | 类型 | 是否必填 | 说明
------------ | ------------- | ------------ | ----------
package_code|String	|Y	|产品码，唯一
package_name|String	|Y	|产品名称
package_desc|String|N|产品描述
details	|Array|	Y|产品明细，
creator|String|Y|创建人
create_time|datetime|Y|创建时间
updater|String|Y|修改人
...
名称 | 类型 | 是否必填 | 说明
------------ | ------------- | ------------ | --------------
package_name | String  | O(可选) | 产品名称
package_name | String  | O(可选) | 产品编码
pageSize|Int|Y|每页显示记录数，默认8
pageNo|Int|Y|当前页号，默认1

样例报文：

	{‘package_name’:’book000001’,
	’package_name’:’书1’,
	'pageSize':8,
	'pageNo':1
	}
...
------------ | ------------- | ------------ | --------------
package_name | String  | Y | 套餐名称
package_code | String  | Y | 套餐编码
package_desc|String|Y|套餐说明
creator|String|Y|创建人
create_time|datetime|Y|创建时间
updater|String|Y|修改人
update_time|datetime|Y|修改时间，update时，永远显示当前时间
...
样例报文：
{‘error’:’Query packages information error.’}