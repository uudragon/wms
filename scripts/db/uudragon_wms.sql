...
CREATE TABLE IF NOT EXISTS `T_PRODUCT_PACKAGE`(
  ...
  `PACKAGE_CODE` VARCHAR(50) NOT NULL COMMENT '产品包编号',
  `PACKAGE_NAME` VARCHAR(100) NOT NULL COMMENT '产品包名称',
  `PACKAGE_DESC` VARCHAR(255) COMMENT '产品包说明',
...
insert into t_product_package(`package_code`,`package_name`,`package_desc`,`creator`, `updater`, `yn`) values('default','默认产品包','系统默认产品包', 'system','system',1);
...
CREATE TABLE IF NOT EXISTS `WMS`.`T_PRODUCT` (
  ...
  `PACKAGE_CODE` VARCHAR(50) NOT NULL COMMENT '产品包编号' DEFAULT 'default',
...
CREATE TABLE IF NOT EXISTS `WMS`.`T_SHIPMENT` (
  ...
  `CUSTOMER_CODE` VARCHAR(36) NOT NULL COMMENT '客户编号',
  ...
  `UPDATE_TIME` timestamp NOT NULL DEFAULT '0000-00-00 00:00:00' COMMENT '实际发货时间',
  `UPDATER` varchar(32) DEFAULT NULL COMMENT '更新人',
  `STATUS` int(11) NOT NULL DEFAULT 0 COMMENT '发货单状态。0：待审核；1：待发货；2：备货中；3：发货中；4：已发货'
  `YN` tinyint(4) NOT NULL COMMENT '是否有效（0：有效；1：无效）',
  
  PRIMARY KEY (`ID`),
  UNIQUE INDEX `SHIPMENT_NO_UNIQUE` (`SHIPMENT_NO`),
  UNIQUE INDEX `EXPRESS_ORDERS_NO_UNIQUE` (`EXPRESS_ORDERS_NO`)
) 
ENGINE=InnoDB 
COMMENT='发货信息表';
...
CREATE TABLE IF NOT EXISTS `WMS`.`T_SHIPMENT_DETAILS` (
  ...
  `TYPE` int(4) NOT NULL DEFAULT 1 COMMENT '明细类型。1：商品；2：赠品；3：发票',
