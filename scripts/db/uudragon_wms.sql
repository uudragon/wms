...
CREATE TABLE IF NOT EXISTS `WMS`.`T_W_PRODUCT_DETAILS`(
  ...
  `CREATEOR` VARCHAR(50) COMMENT '创建人',
  `CREATE_TIME` TIMESTAMP COMMENT '创建时间',
  `UPDATER` VARCHAR(50) COMMENT '修改人',
  `UPDATE_TIME` TIMESTAMP COMMENT '修改时间',
...
CREATE TABLE IF NOT EXISTS `WMS`.`T_SHIPMENT` (
  ...
  `SHIPMENT_NO` varchar(32) NOT NULL COMMENT '发货单号，非空',
  `SHIPPED_QTY` int(11) DEFAULT NULL COMMENT '发货商品总数',
  `EXPRESS_CODE` tinyint(4) NOT NULL COMMENT '快递公司编号',
  `EXPRESS_ORDERS_NO` varchar(100) NOT NULL COMMENT '快递单号，非空',
  `EXPRESS_NAME` varchar(32) DEFAULT NULL COMMENT '快递公司名称',
  `EXPRESS_COST` decimal(10,0) DEFAULT NULL COMMENT '快递费用',
  `COURIER` varchar(32) DEFAULT NULL COMMENT '快递员姓名',
...
CREATE TABLE IF NOT EXISTS `WMS`.`T_SHIPMENT_DETAILS` (
  ...
  `GOODS_CODE` varchar(32) NOT NULL COMMENT '商品编号，非空',
  `GOODS_NAME` varchar(200) NOT NULL COMMENT '商品名称，非空',
  `GOODS_UNIT` varchar(200) DEFAULT NULL COMMENT '商品规格',
  `BARCODE` varchar(50) DEFAULT NULL COMMENT '商品条码',
  `GOODS_QTY` int NOT NULL DEFAULT 1 COMMENT '应发商品数量',
  `ACTUAL_QTY` int NOT NULL DEFAULT 1 COMMENT '实发商品数量',
  `GOODS_DESC` varchar(200) DEFAULT NULL COMMENT '商品描述',
  `YN` tinyint(4) NOT NULL COMMENT '是否有效（0：有效；1：无效）',
  
  PRIMARY KEY (`ID`),
  INDEX `SHIPMENT_NO_UNIQUE` (`SHIPMENT_NO`)
) 
ENGINE=InnoDB 
COMMENT='发货明细表';
