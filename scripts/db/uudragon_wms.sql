...
CREATE TABLE IF NOT EXISTS `T_CONTACT`(
  ...
  `PROV` varchar(64) NOT NULL COMMENT '省',
  `CITY` varchar(64) NOT NULL COMMENT '市',
  `ADDRESS` varchar(255) NOT NULL COMMENT '地址',
  `POST` varchar(20) NOT NULL COMMENT '邮编',
  `TEL` varchar(20) NOT NULL COMMENT '电话',

  PRIMARY KEY(ID),
  UNIQUE KEY `UK_ALL` (`NAME`, `PROV`, `CITY`, `ADDRESS`, `POST`, `TEL`)
)
ENGINE=InnoDB
COMMENT='通讯信息表';

INSERT INTO T_CONTACT(ID, NAME, PROV, CITY, ADDRESS, POST, TEL) VALUES('1', '优优龙（北京）国际教育科技有限公司', '北京', '北京', '北京市朝阳区十里堡恒泰大厦A座6层', '100025', '400');
...
CREATE TABLE IF NOT EXISTS `T_PRODUCT_PACKAGE`(
  ...
  `PACKAGE_PRICE` DECIMAL(20,2) NOT NULL COMMENT '套餐价格' DEFAULT 0.00,
  `PACKAGE_DESC` VARCHAR(255) COMMENT '套餐说明',
  `PACKAGE_TYPE` int(4) NOT NULL COMMENT '套餐类型。1：官网套餐；2：代理商套餐；3：首期套餐' DEFAULT 1,
...
CREATE TABLE IF NOT EXISTS `WMS`.`T_GOODS` (
  `ID` BIGINT NOT NULL AUTO_INCREMENT COMMENT 'ID',
  `GOODS_TYPE` TINYINT(4) COMMENT '商品类型。1：教材（书籍）；2.音像制品（DVD/CD）；3：玩具；4：其他；5：宣传品；6：赠品；',
...
CREATE TABLE IF NOT EXISTS `WMS`.`T_SHIPMENT` (
  ...
  `SOURCE` INT(4) NOT NULL COMMENT '订单来源。1：网站；2：电话；3：代理商代下单；4：代理商订单',
  `AGENT_CODE` varchar(50) COMMENT '所属代理商编号',
  ...
  `PHONE` VARCHAR(20) NULL COMMENT '座机电话',
  `POST` VARCHAR(20) NULL COMMENT '邮编',
  ...
  `BIG_PEN` VARCHAR(100) NULL COMMENT '大头笔信息',
